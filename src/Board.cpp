...
void Board::checkPoint(int row, int column) //checks and deals with one singular stone 
{
  Stone* stone = array[row][column];
  Stone* current = stone;
  Stone* group[size*size];
  ...
  for (int i = 0; i < groupCounter; i++)
  {
    if (group[i]->getUpChild() != 0)
    {
      if (group[i]->getUpChild()->getChar()   == '0')
        team1 = true;
      else if (group[i]->getUpChild()->getChar()   == 'O')
        team2 = true;
    }

    if (group[i]->getDownChild() != 0)
    {
      if (group[i]->getDownChild()->getChar()   == '0')
        team1 = true;
      else if (group[i]->getDownChild()->getChar()   == 'O')
        team2 = true;
    }

    if (group[i]->getLeftChild() != 0)
    {
      if (group[i]->getLeftChild()->getChar()   == '0')
        team1 = true;
      else if (group[i]->getLeftChild()->getChar()   == 'O')
        team2 = true;
    }

    if (group[i]->getRightChild() != 0)
    {
      if (group[i]->getRightChild()->getChar()   == '0')
        team1 = true;
      else if (group[i]->getRightChild()->getChar()   == 'O')
        team2 = true;
    }
    if (team1 && team2)
      safe = true;
    else if (team1 == false && team2 == false)
      safe = true;
  }	
  
  //Deal with scorable group
  if (safe == false)
  {
    for (int i = 0; i < groupCounter; i++)
    {
...
void Board::checkAllPoints(void) // calls checkPoint for each unchecked stone
{
  for(int i = 0; i < size; i++)
    {
      for(int j = 0; j < size; j++)
